import type { Agent, AgentMeta } from './interfaces';

// Demo agents for the logged-out auth-page preview (embedded mode). The live app
// fetches the same shape from GET /v1/metrics/agents. avg_latency_ms is in
...
// Per-agent configuration metadata for the detail page, keyed by agent name.
// Demo-only: there is no live per-agent config endpoint yet, so the detail page
// renders this for every agent (falling back to a representative entry for any
// live agent not listed here). Health and anomaly fields are deliberately
// absent — only configuration and the agent's tool surface live here.
export const AGENT_META: Record<string, AgentMeta> = {
  "summarize-comments": {
    ...
    temperature: 0.2, maxTokens: 1024, timeout: "45s", retries: 1, endpoint: "agents.summarize-comments",
    tools: [
  ...
  "generate-clock-out-description": {
    ...
    temperature: 0.4, maxTokens: 768, timeout: "30s", retries: 2, endpoint: "agents.generate-clock-out-description",
    tools: [
  ...
  "classify-intent": {
    ...
    temperature: 0.0, maxTokens: 128, timeout: "15s", retries: 2, endpoint: "agents.classify-intent",
    tools: [
  ...
  "extract-entities": {
    ...
    temperature: 0.0, maxTokens: 512, timeout: "20s", retries: 1, endpoint: "agents.extract-entities",
    tools: [
  ...
  "rewrite-message": {
    ...
    temperature: 0.3, maxTokens: 512, timeout: "30s", retries: 2, endpoint: "agents.rewrite-message",
    tools: [
  ...
  "detect-sentiment": {
    description: "Scores a single message on a continuous -1 to +1 sentiment axis with an emotion label. Lowest-latency agent in the workspace.",
    ...
    temperature: 0.0, maxTokens: 64, timeout: "10s", retries: 1, endpoint: "agents.detect-sentiment",
    tools: [
  ...
  "summarize-thread": {
    ...
    temperature: 0.3, maxTokens: 1536, timeout: "60s", retries: 1, endpoint: "agents.summarize-thread",
    tools: [
  ...
  "moderate-content": {
    description: "Flags policy-violating content across 9 categories and returns a block/allow/review decision. Highest-volume agent.",
    ...
    temperature: 0.0, maxTokens: 96, timeout: "10s", retries: 2, endpoint: "agents.moderate-content",
    tools: [
...
// Representative fallback for any agent without a dedicated AGENT_META entry
// (e.g. a live agent name not in the demo set).
export const DEFAULT_AGENT_META = AGENT_META["rewrite-message"];
