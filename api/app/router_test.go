...
func TestVerifyRateLimitIsSeparateFromLogin(t *testing.T) {
	...
	router := newRouter(cfg, repo, &mocks.MockWorkspaceStore{}, &mocks.MockInviteStore{}, nil, issuer, auth.NewService(nil, issuer, nil), apikey.NewService(nil), nil, Options{})
...
func TestExtensionsShareAuthAndCannotReplaceCoreRoutes(t *testing.T) {
	...
	repo := repository{mocks.NewMockTraceRepository(), &mocks.MockMetricsRepository{}}
	router := newRouter(cfg, repo, &mocks.MockWorkspaceStore{}, &mocks.MockInviteStore{}, nil, issuer, auth.NewService(nil, issuer, nil), apikey.NewService(nil), nil, opts)
